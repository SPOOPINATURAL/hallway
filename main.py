...
class Ket:
    ...
    def update(self, i_frame, state, event_number, x):
        ...
        window_info = GetMonitorInfo(MonitorFromPoint((0, 0)))
        if window_info['Work'] == GetMonitorInfo(MonitorFromPoint((0, 0)))['Monitor']:
        ...
        # Check and adjust if the window is off-screen
        if self.x < 0:
            self.x = 0
        elif self.x > (screen_width - 128):  # Adjust for the width of the window
            self.x = screen_width - 128

        if self.y < 0:
            self.y = 0
        elif self.y > (work_height - 128):  # Adjust for the height of the window
